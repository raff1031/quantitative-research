# =================================================================================
# XGBoost Stock Prediction - v78 (Focused Test: Biotech + Curated Features)
#
# DESCRIZIONE:
# Test finale mirato che combina tutte le nostre scoperte:
#
# - ⚠️ v78: Test focalizzato solo su MRNA e LLY (dove abbiamo visto un "edge").
# - ⚠️ v78: Utilizza ESCLUSIVAMENTE il "feature set curato" (curated_lite)
#   che include feature di sentiment e l'indice di settore 'biotech_dir'.
# - ⚠️ v78: Mantiene l'ottimizzazione dei quantili (90/10 vs 80/20).
...
config = {
    ...
    "N_CANDIDATE_STRATEGIES": 2, # Ora abbiamo solo 2 candidati (90/10 e 80/20)

    # --- Model & Feature Parameters ---
    "SEED": 42,
    "FRAC_DIFF_D": 0.5,
    "WINDOW_LIST_TO_TEST": [60], # Testiamo solo 1 finestra per velocizzare
    
    # ⚠️ v78: Testiamo solo il nostro set curato
    "FEATURE_COUNTS_TO_TEST": ['curated_lite'], 
    ...
    # ⚠️ v78: Elenco di quantili da testare
    ...
    # --- Asset & File Paths ---
    # ⚠️ v78: Test focalizzato su MRNA e LLY
    "TICKERS": ['MRNA', 'LLY'],
    ...
    "TOP_N_CANDIDATES_FOR_CSV": 2, # Salva entrambi i risultati dei quantili
...
# --- Setup ---
CACHE_DIR = "cache_v78.1_backtester" # ⚠️ v78
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v78.1_biotech_curated_{datetime.today().strftime('%Y%m%d_%H%M%S')}" # ⚠️ v78
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v78.1.log') # ⚠️ v78
...
def load_and_prepare_features():
    # ⚠️ v78: Assicurati che TUTTI i ticker necessari siano scaricati
    ...
    if os.path.exists(FULL_FEATURES_CACHE_PATH) and not config["FORCE_RECALCULATE_FEATURES"]:
        ...
        df_seasonal = create_seasonality_features(df_close.index)
        # ⚠️ v78: Passa l'elenco completo dei ticker a get_sentiment_scores
        ...
        # ⚠️ v78: Assicurati di calcolare ts_mom e rv per tutti i ticker rilevanti
    ...
    # ⚠️ v78: Calcola i target per *tutti* gli asset, non solo quelli dello smoke test
...
def main():
        ...
        for window_size_val in config["WINDOW_LIST_TO_TEST"]:
            ...
            for n_features in config["FEATURE_COUNTS_TO_TEST"]:
                ...
                parallel_results = Parallel(n_jobs=n_jobs_validation, backend='loky')(
                    ...
                    ) for symbol in config["TICKERS"] # ⚠️ v78: Esegue solo su ['MRNA', 'LLY']
                )
    ...
    with open(final_report_path, 'w', encoding='utf-8') as f:
        # ⚠️ v78: Assicurati che i ticker del B&H siano corretti
        all_bh_tickers = list(set(config["TICKERS"] + ['AAPL', 'MRNA', 'LLY'])) # Aggiungi tutti i ticker rilevanti
