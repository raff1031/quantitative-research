# =================================================================================
# XGBoost Stock Prediction - v81 (Final Biotech Optimization)
#
# DESCRIZIONE:
# Test finale e mirato. Applica l'intera pipeline di ottimizzazione
# (finestre, # feature, quantili) ESCLUSIVAMENTE sugli asset
# e sulla strategia che si sono dimostrati promettenti.
#
# - ⚠️ v81: Test focalizzato solo su MRNA e LLY.
# - ⚠️ v81: Test ESCLUSIVO della strategia 'Single Day' (Trend Following rimossa).
# - ⚠️ v81: Ri-attivata la selezione delle feature [20, 40, ..., 'all']
#   per trovare il set di feature ottimale per il biotech.
# - ⚠️ v81: Mantiene l'ottimizzazione dei quantili (90/10 vs 80/20).
...
config = {
    ...
    "N_CANDIDATE_STRATEGIES": 10, # Selezioniamo le top 10 combinazioni
    ...
    # ⚠️ v81: Testiamo di nuovo tutti i set di feature
    ...
    # ⚠️ v81: Elenco di quantili da testare
    ...
    # --- Asset & File Paths ---
    # ⚠️ v81: Test focalizzato su MRNA e LLY
    "TICKERS": ['MRNA', 'LLY'],
    ...
    "TOP_N_CANDIDATES_FOR_CSV": 3, # Salva i top 3 candidati
...
# --- Setup ---
CACHE_DIR = "cache_v81_backtester" # ⚠️ v81
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v81_biotech_final_{datetime.today().strftime('%Y%m%d_%H%M%S')}" # ⚠️ v81
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v81.log') # ⚠️ v81
...
def run_backtest_evaluation(strategy_fn, symbol, df_features_base, df_targets, df_regimes, df_close, df_high, df_low, X_main_idx, X_unseen_idx, isee_z_col, isee_regime_col, n_features, df_seasonal, df_macro_ratios, strategy_name="Overnight"):
    ...
    # ⚠️ v78: Logica di feature selection "curated_lite"
...
def run_backtest_for_symbol(symbol, df_features_for_run, df_targets, df_regimes, df_close, df_high, df_low, n_features, window_size, train_idx, test_idx, df_seasonal, df_macro_ratios):
    ...
    # ⚠️ v81: Esegui solo la strategia Single Day (Overnight)
    result_sds = run_backtest_evaluation(strategy_fn=strategy_single_day_swing, **{**common_args, "strategy_name": "Overnight"})
    
    # Ritorna i risultati solo per single_day, impostando trend_following a None
    return {symbol: {'single_day': result_sds, 'trend_following': None}}


def load_and_prepare_features():
    # ⚠️ v81: Assicurati che TUTTI i ticker necessari siano scaricati
    all_tickers_in_config = list(set(
        config["TICKERS"] + 
        ['AAPL', 'GOOG', 'TSLA', '^IXIC', 'MRNA', 'LLY', 'ETH-USD', 'SOL-USD'] + # Assicura che i ticker base ci siano
    ...
    if os.path.exists(FULL_FEATURES_CACHE_PATH) and not config["FORCE_RECALCULATE_FEATURES"]:
        ...
        df_seasonal = create_seasonality_features(df_close.index)
        # ⚠️ v81: Passa l'elenco completo dei ticker a get_sentiment_scores
        ...
        # ⚠️ v81: Assicurati di calcolare ts_mom e rv per tutti i ticker rilevanti
    ...
    # ⚠️ v81: Calcola i target per *tutti* gli asset, non solo quelli dello smoke test
...
def save_trades_to_csv(symbol, strategy_name, rank, trades_df, total_return, max_dd, calmar_ratio, config_info, output_dir):
    ...
    # Crea nome file
    # ⚠️ v78: Aggiungi i quantili al nome del file per distinguerli
...
def main():
        ...
        for window_size_val in config["WINDOW_LIST_TO_TEST"]:
            ...
            for n_features in config["FEATURE_COUNTS_TO_TEST"]:
                ...
                parallel_results = Parallel(n_jobs=n_jobs_validation, backend='loky')(
                    ...
                    ) for symbol in config["TICKERS"] # ⚠️ v81: Esegue solo su ['MRNA', 'LLY']
                )
    ...
    for symbol in config["TICKERS"]:
        ...
        for strategy_type, candidates in candidate_strategies[symbol].items():
            ...
            # ⚠️ v81: Salta 'trend_following'
            if strategy_type == 'trend_following':
                logging.info(f"[{symbol}] Salto la strategia 'Trend Following' come da v81.")
                continue
    ...
    with open(final_report_path, 'w', encoding='utf-8') as f:
        # ⚠️ v81: Assicurati che i ticker del B&H siano corretti
