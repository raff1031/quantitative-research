...
// Strategy: HourSweepRetracementInverse
// Versione: con filtro trend Bollinger e break-even - LOGICA BUY/SELL INVERTITA
...
{
    public class HourSweepRetracementInverse : Strategy
    {
        // ---- Timezones ----
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Name                                    = "HourSweepRetracementInverse"; // Modificato il nome della strategia
                Description                             = "Logica INVERTITA: Sweep+Return-to-Open orario; BB trend filter e break-even opzionali.";
        ...
        protected override void OnBarUpdate()
        {
            ...
            // ---- SWEEP e ingresso (LOGICA INVERTITA) ----
            bool entered = false;
            if (High[0] > prevHourHigh && allowShortByBB) // SWEEP UP -> Entra LONG (logica invertita)
            {
                if (DebugMode) Print($"ET {tET:HH:mm} | SWEEP UP -> LONG (Invertito)");
                if (UseATRExit)
                {
            ...
            else if (Low[0] < prevHourLow && allowLongByBB) // SWEEP DOWN -> Entra SHORT (logica invertita)
            {
                if (DebugMode) Print($"ET {tET:HH:mm} | SWEEP DOWN -> SHORT (Invertito)");
                if (UseATRExit)
                {
