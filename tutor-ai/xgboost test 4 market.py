# =================================================================================
# XGBoost Stock Prediction - v73 (FIXED Lookahead Bias)
#
# DESCRIZIONE:
...
# - Chunking temporale per Google Trends (evita errore 429)
# - ⚠️ FIX v73: Correzione del lookahead bias applicando shift(1) alle posizioni.
# =================================================================================
...
config = {
    ...
    "GOOGLE_TRENDS_CHUNK_MONTHS": 12,  # Chunk size in months (12 = 1 year per request)
    "GOOGLE_TRENDS_CHUNK_DELAY": 1.5,  # Extra delay (seconds) between chunks
...
# --- Setup ---
CACHE_DIR = "cache_v73_backtester" # ⚠️ v73
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v73_final_backtester_{datetime.today().strftime('%Y%m%d_%H%M%S')}" # ⚠️ v73
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v73.log') # ⚠️ v73
...
def get_google_trends_data(keywords, index, z_score_window, run_new_analysis=False, min_delay=2.5, max_delay=4.0, max_retries=5, chunk_months=12, chunk_delay=1.5):
    ...
    # Crea chunk temporali (es: 2015, 2016, 2017, ...)
                ...
                try:
                    ...
                    if not interest_over_time_df.empty and keyword in interest_over_time_df.columns:
                        ...
                        break  # Successo, passa al prossimo chunk
                         ...
                         break # Successo (ma vuoto), passa al prossimo chunk
                    else:
                        # Caso in cui interest_over_time_df è vuoto
                        logging.info(f"  [OK] '{keyword}' - Chunk {chunk_idx+1}/{len(date_chunks)} - Nessun dato (vuoto).")
                        break
...
def calculate_trade_metrics(positions, close_prices, high_prices, low_prices, actual_returns, commission_rate):
    ...
    IMPORTANTE: Questa funzione ora riceve posizioni GIÀ SHIFTATE (spostate).
    """
    trades = []
    current_position = 0
    entry_date = None
    ...
    # 'positions' qui sono GIÀ shiftate di 1, quindi questo calcolo è corretto.
    ...
    for date in positions.index:
        ...
        if new_position != current_position:
            ...
            if current_position != 0 and entry_date is not None:
                # Usa il prezzo del giorno CORRENTE (date) per uscire
                ...
                # NOTA: La commissione di entrata è già stata pagata
                # Questa logica calcola il PnL del trade *complessivo*
                commission_total = commission_rate # Commissione di uscita
                
                # Il PnL netto deve considerare entrambe le commissioni
                pnl_net = pnl_gross - (2 * commission_rate)


                # Calculate holding time
                holding_time = (exit_date - entry_date).days
                
                # Se holding_time è 0, è un day trade.
                # Questo è corretto perché stiamo usando 'date' sia per
                # l'entrata (nel 'if new_position != 0' sotto)
                # sia per l'uscita (qui sopra).
                # Un trade aperto e chiuso nello stesso giorno avrà holding_time = 0.
                if holding_time == 0:
                    logging.debug(f"Day trade (0 holding time) rilevato: {entry_date} -> {exit_date}")
                ...
                trades.append({
                    ...
                    'commission': (2 * commission_rate) * 100, # Commissione totale
                    'pnl_net': pnl_net * 100,
            ...
            if new_position != 0:
                entry_date = date
                entry_price = close_prices.loc[date] # Entra al prezzo del giorno CORRENTE
            else:
...
def strategy_single_day_swing(predictions, actual_returns, **kwargs):
    ...
    # This aligns the signal from Day T with the return of Day T+1
...
def run_backtest_evaluation(strategy_fn, symbol, df_features_base, df_targets, df_regimes, df_close, df_high, df_low, X_main_idx, X_unseen_idx, isee_z_col, isee_regime_col, n_features, df_seasonal, df_macro_ratios, strategy_name="Overnight"):
    ...
    # actual_returns=df_close[symbol].pct_change() è il ritorno T-1 -> T, disponibile a T.
    # Le nostre funzioni di strategia ora applicano shift(1), quindi
    # la posizione di T (basata su dati T-1) è moltiplicata per il ritorno di T.
...
def main():
    ...
    if os.path.exists(CHECKPOINT_PATH):
        ...
        try:
            ...
            logging.warning(f"Errore nel caricamento del checkpoint di validazione: {e}. Ricalcolo da zero.")
            all_validation_results = {}
