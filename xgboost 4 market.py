...
config = {
    ...
    "NEWS_FILES": {
        "TECH_NEWS": "Apple_News_New_Format.xlsx", "MRNA": "moderna_news.xlsx",
        "LLY": "Documents Headlines (3256) - 27-06-2025 LLY.xlsx", "GOOG": "goog_news.xlsx",
        "TSLA": "tsla_news.xlsx",
    },
...
# --- Setup ---
CACHE_DIR = "cache_v72.4"
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v72.4_final_{datetime.today().strftime('%Y%m%d_%H%M%S')}"
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v72.4.log')
...
def get_sentiment_scores(symbols, news_files, index, run_new_analysis=False):
    if not run_new_analysis and os.path.exists(config["COMPUTED_SENTIMENT_PATH"]):
        ...
        logging.info("Esecuzione nuova analisi del sentiment (logica omessa)...")
        return pd.DataFrame(index=index)
    ...
    logging.info("Applicazione correzione Data Leakage per le news (shift di 1 giorno)...")
    ...
    for s in symbols:
        if f"{s}_Sentiment_Formal" not in final_df.columns: final_df[f"{s}_Sentiment_Formal"] = 0.0
        if f"{s}_Sentiment_Social" not in final_df.columns: final_df[f"{s}_Sentiment_Social"] = 0.0
    return final_df

def get_google_trends_data(keywords, index, z_score_window, run_new_analysis=False):
    ...
    logging.info("Scaricamento di nuovi dati da Google Trends con retry/backoff...")
    ...
    for keyword in tqdm(keywords, desc="Analisi Google Trends per keyword"):
        for attempt in range(5):
            try:
                ...
                if attempt < 4: time.sleep(wait_time)
                else: logging.error(f"Impossibile scaricare dati per '{keyword}' dopo 5 tentativi.")
...
def run_backtest_evaluation(strategy_fn, symbol, df_features_base, df_targets, df_regimes, df_close, X_main_idx, X_unseen_idx, isee_z_col, isee_regime_col, n_features, df_seasonal, df_macro_ratios, strategy_name="Overnight"):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    xgb_params = {'objective': 'reg:squarederror', 'random_state': config["SEED"], 'device': device}
    ...
    if all_predictions.empty:
        ...
        return None
    strategy_returns, _ = strategy_fn(
    ...
    return {"total_strategy_return": total_strategy_return, "max_drawdown": max_dd, "calmar_ratio": calmar_ratio, "cumulative_returns": cumulative_strategy_returns}

def run_backtest_for_symbol(symbol, df_features_for_run, df_targets, df_regimes, df_close, n_features, window_size, train_idx, test_idx, df_seasonal, df_macro_ratios):
    ...
    common_args = {
        ...
        "df_regimes": df_regimes, "df_close": df_close, "X_main_idx": train_idx,
        "X_unseen_idx": test_idx, "isee_z_col": ISEE_MOMENTUM_Z_COL,
...
def load_and_prepare_features():
    if os.path.exists(FULL_FEATURES_CACHE_PATH) and not config["FORCE_RECALCULATE_FEATURES"]:
        ...
        df_close, _, _, _, _ = download_data_with_fallback(all_tickers_to_download, config["START_DATE"], config["END_DATE"])
        ...
        df_trends_z = get_google_trends_data(config["TRENDS_KEYWORDS"], df_close.index, config["TRENDS_Z_SCORE_WINDOW"], run_new_analysis=config["RUN_NEW_TRENDS_ANALYSIS"])
    ...
    return df_features_base, df_targets, df_regimes, df_close, df_seasonal, df_macro_ratios

def main():
    np.random.seed(config["SEED"]); random.seed(config["SEED"]); torch.manual_seed(config["SEED"])
    tree_method = 'gpu_hist' if torch.cuda.is_available() else 'hist'
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(config["SEED"]); torch.backends.cudnn.deterministic = True; torch.backends.cudnn.benchmark = False
        logging.info("***** CUDA (GPU) è disponibile. Lo script utilizzerà la GPU per l'addestramento di XGBoost. *****")
    else:
        logging.info("***** CUDA (GPU) non disponibile. Lo script utilizzerà la CPU per l'addestramento di XGBoost. *****")

    df_features_base, df_targets, df_regimes, df_close, df_seasonal, df_macro_ratios = load_and_prepare_features()
        ...
        for n_features in config["FEATURE_COUNTS_TO_TEST"]:
            ...
            parallel_results = Parallel(n_jobs=n_jobs_validation, backend='loky')(
                delayed(run_backtest_for_symbol)(
                    symbol, df_features_for_run, df_targets, df_regimes, df_close, n_features, window_size_val,
