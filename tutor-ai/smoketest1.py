# =================================================================================
# XGBoost Stock Prediction - v75 (Wider Quantiles)
#
# DESCRIZIONE:
# Questa versione prende i risultati positivi della v74 (feature stazionarie)
# e allenta i quantili per cercare di catturare più ritorni.
#
# - ⚠️ v75: Quantili allargati a 80/20 (da 90/10)
# - ⚠️ v75: Smoke test rimosso, tutti gli 8 asset sono riattivati.
...
config = {
    ...
    # ⚠️ v75: Quantili allentati
    "QUANTILE_BUY": 0.80,
    "QUANTILE_SHORT": 0.20,

    # --- Asset & File Paths ---
    # ⚠️ v75: Smoke test rimosso, tutti gli asset riattivati
...
# --- Setup ---
CACHE_DIR = "cache_v75_backtester" # ⚠️ v75
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v75_wider_quantiles_{datetime.today().strftime('%Y%m%d_%H%M%S')}" # ⚠️ v75
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v75.log') # ⚠️ v75
...
def calculate_features_for_ticker(s, df_open, df_high, df_low, df_close, df_volume):
    ...
    return df_ta
# ==============================================================================
# --- (Fine della funzione modificata) ---
# ==============================================================================
...
def strategy_single_day_swing(predictions, actual_returns, **kwargs):
    ...
    # Questo allinea il segnale del Giorno T con il ritorno del Giorno T+1
...
def run_backtest_evaluation(strategy_fn, symbol, df_features_base, df_targets, df_regimes, df_close, df_high, df_low, X_main_idx, X_unseen_idx, isee_z_col, isee_regime_col, n_features, df_seasonal, df_macro_ratios, strategy_name="Overnight"):
    ...
    # Rimuovi colonne con varianza zero (costanti) che farebbero crashare il training
    ...
    # ⚠️ v74: Aggiorna le keyword di esclusione per matchare i nuovi nomi delle feature (es. '_diff')
    ...
    # Assicurati che tutte le feature selezionate esistano ancora dopo la pulizia della varianza
    ...
    for i, chunk_idx in enumerate(tqdm(unseen_chunks_indices, desc=f"WF Predictions for {symbol}", leave=False, position=1)):
        if i % config["REOPTIMIZATION_FREQUENCY_MONTHS"] == 0:
            ...
            # Applica la stessa logica di pulizia
            X_train_wf = X_train_wf.reindex(columns=final_feature_list, fill_value=0)
            non_constant_cols_wf = X_train_wf.columns[X_train_wf.nunique() > 1]
            X_train_wf_selected = X_train_wf[non_constant_cols_wf]
        ...
        if final_model:
            ...
            # Assicurati che le colonne del test corrispondano a quelle usate per l'addestramento
...
def load_and_prepare_features():
    # ⚠️ v75: Assicurati che TUTTI i ticker necessari siano scaricati
    all_tickers_in_config = list(set(
        ...
        ['AAPL', 'GOOG', 'TSLA', '^IXIC', 'MRNA', 'LLY', 'ETH-USD', 'SOL-USD'] # Assicurati che i ticker base ci siano
    ))
    ...
    if os.path.exists(FULL_FEATURES_CACHE_PATH) and not config["FORCE_RECALCULATE_FEATURES"]:
        ...
        df_advanced_tech = get_or_calculate_technical_indicators(df_close, df_high, df_low, df_open, df_volume) # ⚠️ v74: Questa ora calcola feature stazionarie
    ...
    for window_size_val in config["WINDOW_LIST_TO_TEST"]:
        ...
        for n_features in config["FEATURE_COUNTS_TO_TEST"]:
            ...
            parallel_results = Parallel(n_jobs=n_jobs_validation, backend='loky')(
                ...
                ) for symbol in config["TICKERS"] # ⚠️ v75: Ora esegue su tutti gli 8 asset
            )
