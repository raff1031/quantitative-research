# =================================================================================
# XGBoost Stock Prediction - v72.7 (Google Trends Chunking)
#
# DESCRIZIONE:
...
# - NUOVO v3: Chunking temporale per Google Trends (evita errore 429)
# =================================================================================
...
# --- Setup ---
CACHE_DIR = "cache_v72.7_backtester"
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v72.7_final_backtester_{datetime.today().strftime('%Y%m%d_%H%M%S')}"
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v72.7.log')
...
def get_google_trends_data(keywords, index, z_score_window, run_new_analysis=False, min_delay=2.5, max_delay=4.0, max_retries=5, chunk_months=12, chunk_delay=1.5):
    ...
    Suddivide il periodo totale in chunk (default: 12 mesi = 1 anno) e scarica separatamente.
    Questo riduce drasticamente il rischio di rate limiting.
    ...
    for keyword in tqdm(keywords, desc="Analisi Google Trends per keyword"):
        ...
        if keyword_data_chunks:
            ...
            logging.info(f"[WARN] '{keyword}' - Completato: {len(keyword_full)} data points")
        else:
