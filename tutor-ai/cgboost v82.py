# =================================================================================
# XGBoost Stock Prediction - v82 (Production-Ready Optimization)
#
# DESCRIZIONE:
# Versione migliorata con correzioni critiche e ottimizzazioni:
# - ✅ FIX: Variabili ambiente OMP/MKL corrette
# - ✅ FIX: Quantili passati come parametri (no race conditions)
# - ✅ NUOVO: Position sizing dinamico basato su volatilità
# - ✅ NUOVO: Metriche aggiuntive (Sharpe, Sortino, Win Rate, Profit Factor)
# - ✅ NUOVO: Commissioni realistiche per biotech (0.15%)
# - ✅ NUOVO: Riduzione overfitting (meno combinazioni, early stopping)
# - ✅ REFACTOR: Codice strategie unificato
# - ✅ REFACTOR: Magic numbers in config
# - Target: MRNA e LLY (biotech focus)
# - Strategia: Single Day Swing
# =================================================================================
...
# --- ✅ FIX v82: Corrette variabili ambiente (era '-1', errore critico) ---
...
config = {
    # --- Execution Control ---
    # ✅ v82: False = usa cache se disponibile. Metti True solo se hai nuovi dati.
    "RUN_NEW_SENTIMENT_ANALYSIS": False,
    "RUN_NEW_TRENDS_ANALYSIS": False,
    "RUN_NEW_HMM_ANALYSIS": False,
    "FORCE_RECALCULATE_FEATURES": False,

    # --- Robust Validation Parameters ---
    "OOS_MONTHS": 6,
    "VALIDATION_MONTHS": 12,
    "N_CANDIDATE_STRATEGIES": 5,  # ✅ v82: Ridotto da 10 a 5 per ridurre overfitting

    # --- Model & Feature Parameters ---
    "SEED": 42,
    "FRAC_DIFF_D": 0.5,

    # ✅ v82: ISEE finestre ridotte a 3 (invece di 6) per ridurre overfitting
    "WINDOW_LIST_TO_TEST": [30, 60, 90],

    # ✅ v82: Feature counts ridotti a 4 configurazioni (invece di 7)
    "FEATURE_COUNTS_TO_TEST": [40, 80, 150, 'all'],
    ...
    # Quantili da testare (default values, passeremo come parametri)
    ...
    "NEWS_FILES": {
        "TECH_NEWS": r"c:\Users\sas\Desktop\correlation chat goat\News - Headlines ( 9966 ) apple - 29-Jun-25.xlsx",
        "MRNA":      r"c:\Users\sas\Desktop\tutor ai\mrna_news.xlsx",
        "LLY":       r"c:\Users\sas\Desktop\tutor ai\lly_news.xlsx",
        "GOOG":      r"c:\Users\sas\Desktop\correlation chat goat\goog_news.xlsx",
        "TSLA":      r"c:\Users\sas\Desktop\correlation chat goat\tsla_news.xlsx",
    },
    "ISEE_FILE_PATH": r"c:\Users\sas\Desktop\correlation chat goat\ISEE.csv",
    "COMPUTED_SENTIMENT_PATH": r"c:\Users\sas\Desktop\correlation chat goat\Dual_Sentiment_Analysis_Results.csv",
    ...
    # ✅ v82: Commissioni aumentate a 0.15% (più realistico per biotech)
    "COMMISSION_RATE": 0.0015,
    "TOP_N_CANDIDATES_FOR_CSV": 3,

    # ✅ v82: Nuovi parametri per position sizing
    "POSITION_SIZING_METHOD": "volatility_target",  # Options: 'fixed', 'volatility_target', 'kelly'
    "VOLATILITY_TARGET": 0.15,  # Target annuale 15%
    "MAX_POSITION_SIZE": 1.0,  # Massimo 100% del capitale
    "MIN_POSITION_SIZE": 0.2,  # Minimo 20% del capitale

    # ✅ v82: Parametri strategie configurabili
    "QUANTILE_ROLLING_WINDOW": 60,
    "QUANTILE_MIN_PERIODS": 20,
...
# --- Setup ---
CACHE_DIR = "cache_v82_production"
os.makedirs(CACHE_DIR, exist_ok=True)
output_dir = f"run_v82_production_{datetime.today().strftime('%Y%m%d_%H%M%S')}"
os.makedirs(output_dir, exist_ok=True)

log_file_path = os.path.join(output_dir, 'backtest_v82.log')
...
def download_data_with_fallback(symbols, start, end):
    ...
    # ✅ v82: TRENDS_CACHE_PATH escluso — i dati Trends sono indipendenti dai prezzi di mercato
    for path in [TECH_INDICATORS_CACHE_PATH, GARCH_FEATURES_CACHE_PATH, HMM_REGIMES_CACHE_PATH, FULL_FEATURES_CACHE_PATH]:
    ...
    for symbol in tqdm(symbols, desc="Scaricamento dati di mercato"):
        try:
            ...
            logging.error(f"Errore download {symbol}: {e}")
    ...
    if not run_new_analysis and os.path.exists(config["COMPUTED_SENTIMENT_PATH"]):
        ...
        for file_key, file_path in news_files.items():
            ...
            try:
                ...
                # ✅ v82: Supporto sia per 'identifier(s)' (formato vecchio) che 'id' (formato tuoi file)
                id_col = None
                if 'identifier(s)' in news_df.columns:
                    id_col = 'identifier(s)'
                elif 'id' in news_df.columns:
                    id_col = 'id'

                date_col_raw = 'date/time' if 'date/time' in news_df.columns else None
                headline_col_raw = 'headline' if 'headline' in news_df.columns else None

                if date_col_raw and id_col and headline_col_raw:
                    temp_df = news_df[[date_col_raw, id_col, headline_col_raw]].copy()
                    temp_df.dropna(subset=[id_col, headline_col_raw], inplace=True)
                    temp_df.rename(columns={date_col_raw: 'date', headline_col_raw: 'title'}, inplace=True)

                    temp_df['stock_list'] = temp_df[id_col].astype(str).str.split(',')
...
def get_google_trends_data(keywords, index, z_score_window, run_new_analysis=False, min_delay=2.5, max_delay=4.0, max_retries=5, chunk_months=12, chunk_delay=1.5):
    """
    Scarica dati Google Trends con chunking temporale per evitare errore 429.
    ...
    if not run_new_analysis and os.path.exists(TRENDS_CACHE_PATH):
        ...
        try:
            return pd.read_parquet(TRENDS_CACHE_PATH)
        except Exception as e:
            logging.warning(f"Errore lettura cache Trends: {e}. Ricalcolo.")
...
def calculate_features_for_ticker(s, df_open, df_high, df_low, df_close, df_volume):
    ...
    if macd is not None and not macd.empty:
        df_ta['MACD_12_26_9_diff'] = macd.iloc[:, 0].diff()
        df_ta['MACDh_12_26_9_diff'] = macd.iloc[:, 1].diff()
        df_ta['MACDs_12_26_9_diff'] = macd.iloc[:, 2].diff()
    ...
    if bbands is not None and not bbands.empty:
        ...
        df_ta['BBB_20_2.0'] = bbands.iloc[:, 3]
        df_ta['BBP_20_2.0'] = bbands.iloc[:, 4]
...
def get_advanced_technical_indicators_parallel(df_close, df_high, df_low, df_open, df_volume):
    logging.info("Calcolo indicatori tecnici STAZIONARI (CPU-Parallelized)...")
...
def get_or_calculate_technical_indicators(df_close, df_high, df_low, df_open, df_volume):
    if os.path.exists(TECH_INDICATORS_CACHE_PATH) and not config["FORCE_RECALCULATE_FEATURES"]:
        ...
        try:
            return pd.read_parquet(TECH_INDICATORS_CACHE_PATH)
        except Exception as e:
            logging.warning(f"Errore lettura cache indicatori: {e}. Ricalcolo.")
...
def get_garch_features(df_close):
    if os.path.exists(GARCH_FEATURES_CACHE_PATH):
        ...
        try:
            return pd.read_parquet(GARCH_FEATURES_CACHE_PATH)
        except Exception as e:
            logging.warning(f"Errore lettura cache GARCH: {e}. Ricalcolo.")
    ...
    def calculate_garch_for_ticker(s):
        ...
        try:
            ...
            logging.warning(f"Errore GARCH per {s}: {e}")
            return None
...
def fit_predict_single_hmm(train_data, predict_data, n_regimes=3):
    ...
    try:
        ...
        logging.warning(f"Errore HMM: {e}")
        return pd.Series(0, index=predict_data.index)
...
# ✅ v82: Nuove funzioni per calcolare metriche aggiuntive
...
def calculate_profit_factor(trades_df):
    """Calcola profit factor dai trade"""
    if trades_df is None or trades_df.empty:
        return 0.0
...
# ✅ v82: Funzione per position sizing dinamico
def calculate_position_size(symbol, df_close, method='volatility_target', target_vol=0.15, lookback=21):
    """
    Calcola il sizing dinamico della posizione basato su volatilità

    Args:
        symbol: Ticker symbol
        df_close: DataFrame con i prezzi di chiusura
        method: Metodo di sizing ('fixed', 'volatility_target', 'kelly')
        target_vol: Volatilità target annuale (per volatility_target)
        lookback: Periodo di lookback per calcolare volatilità

    Returns:
        Series con i size delle posizioni (da moltiplicare per segnale long/short)
    """
    if method == 'fixed':
        return pd.Series(1.0, index=df_close.index)

    elif method == 'volatility_target':
        # Calcola volatilità realizzata
        returns = df_close[symbol].pct_change()
        realized_vol = returns.rolling(window=lookback).std() * np.sqrt(252)

        # Position size = target_vol / realized_vol
        position_size = (target_vol / realized_vol).replace([np.inf, -np.inf], np.nan)
        position_size = position_size.fillna(method='ffill').fillna(1.0)
        ...
        return position_size.shift(1).fillna(config["MIN_POSITION_SIZE"])
...
def calculate_trade_metrics(positions, close_prices, high_prices, low_prices, actual_returns, commission_rate):
    ...
    IMPORTANTE: Questa funzione riceve 'positions' che sono GIÀ SHIFTATE.
    """
    trades = []
    current_position = 0
    entry_date = None
...
# =============================================================================
# --- ✅ v82: FUNZIONE STRATEGIA UNIFICATA (REFACTORED) ---
# =============================================================================

def apply_strategy_logic(predictions, actual_returns, quantile_buy, quantile_short, hold_positions=False, **kwargs):
    """
    Logica unificata per strategie long/short.

    Args:
        predictions: Serie di predizioni del modello
        actual_returns: Ritorni effettivi
        quantile_buy: Quantile per segnali long
        quantile_short: Quantile per segnali short
        hold_positions: Se True, mantiene posizioni (trend-following), altrimenti chiude giornalmente
        **kwargs: close_prices, high_prices, low_prices per metriche trade

    Returns:
        (strategy_returns, trades_df)
    """
    # Usa parametri configurabili invece di magic numbers
    rolling_window = config["QUANTILE_ROLLING_WINDOW"]
    min_periods = config["QUANTILE_MIN_PERIODS"]

    rolling_buy_thresh = predictions.rolling(window=rolling_window, min_periods=min_periods).quantile(quantile_buy)
    rolling_short_thresh = predictions.rolling(window=rolling_window, min_periods=min_periods).quantile(quantile_short)
    ...
    if hold_positions:
        # Trend-following: mantiene posizioni
        positions = signals.replace(0, np.nan).ffill().fillna(0)
    else:
        # Day trading: chiude posizioni giornalmente
        positions = signals

    # FIX v73: SHIFT POSITIONS BY 1 to prevent lookahead bias
    shifted_positions = positions.shift(1).fillna(0)

    # ✅ v82: Applica position sizing se disponibile
    symbol = kwargs.get('symbol')
    df_close_full = kwargs.get('df_close_full')

    if symbol and df_close_full is not None and symbol in df_close_full.columns:
        ...
        # Applica sizing: segnale (-1, 0, 1) * size (0.2 - 1.0)
        shifted_positions = shifted_positions * position_sizes.loc[shifted_positions.index]
    ...
    if close_prices is not None and high_prices is not None and low_prices is not None:
        ...
        strategy_returns = shifted_positions * actual_returns.loc[shifted_positions.index]
        return strategy_returns.dropna(), None

def strategy_single_day_swing(predictions, actual_returns, quantile_buy, quantile_short, **kwargs):
    """Day trading strategy: opens and closes positions daily."""
    return apply_strategy_logic(predictions, actual_returns, quantile_buy, quantile_short, hold_positions=False, **kwargs)

def strategy_trend_following(predictions, actual_returns, quantile_buy, quantile_short, **kwargs):
    """Momentum/Trend-following strategy: holds positions until opposite signal."""
    return apply_strategy_logic(predictions, actual_returns, quantile_buy, quantile_short, hold_positions=True, **kwargs)

# =============================================================================
# --- ✅ v82: FUNZIONE BACKTEST CON METRICHE MIGLIORATE ---
# =============================================================================

def run_backtest_evaluation(strategy_fn, symbol, df_features_base, df_targets, df_regimes, df_close, df_high, df_low, X_main_idx, X_unseen_idx, isee_z_col, isee_regime_col, n_features, df_seasonal, df_macro_ratios, quantile_buy, quantile_short, strategy_name="Overnight"):
    """
    ✅ v82: Quantili passati come parametri invece di leggere da config globale
    """
    ...
    strategy_returns, trades_df = strategy_fn(
        ...
        symbol=symbol,  # ✅ v82: Per position sizing
        df_close_full=df_close  # ✅ v82: Per position sizing
    )
    ...
    # ✅ v82: Calcola metriche aggiuntive
    sharpe_ratio = calculate_sharpe_ratio(final_strategy_daily_returns)
    sortino_ratio = calculate_sortino_ratio(final_strategy_daily_returns)
    ...
    return {
        ...
        "sharpe_ratio": sharpe_ratio,  # ✅ v82
        "sortino_ratio": sortino_ratio,  # ✅ v82
        "win_rate": win_rate,  # ✅ v82
        "profit_factor": profit_factor,  # ✅ v82
        "cumulative_returns": cumulative_strategy_returns,
        "trades_df": trades_df
    }

# =============================================================================
# --- ✅ v82: FUNZIONE RUN BACKTEST CON QUANTILI PARAMETRIZZATI ---
# =============================================================================
def run_backtest_for_symbol(symbol, df_features_for_run, df_targets, df_regimes, df_close, df_high, df_low, n_features, window_size, quantile_buy, quantile_short, train_idx, test_idx, df_seasonal, df_macro_ratios):
    """
    ✅ v82: Quantili passati come parametri espliciti invece di leggere dalla config globale
    """
    logging.info(f"Starting backtest for {symbol} (Win={window_size}, Feats={n_features}, Q={quantile_buy}/{quantile_short})")
    ...
    common_args = {
        ...
        "quantile_buy": quantile_buy,  # ✅ v82: Passa come parametro
        "quantile_short": quantile_short  # ✅ v82: Passa come parametro
    }

    result_sds = run_backtest_evaluation(strategy_fn=strategy_single_day_swing, **{**common_args, "strategy_name": "Overnight"})

    return {symbol: {'single_day': result_sds, 'trend_following': None}}


def load_and_prepare_features():
    all_tickers_in_config = list(set(
        config["TICKERS"] +
        ['AAPL', 'GOOG', 'TSLA', '^IXIC', 'MRNA', 'LLY', 'ETH-USD', 'SOL-USD'] +
...
def save_trades_to_csv(symbol, strategy_name, rank, trades_df, total_return, max_dd, calmar_ratio, sharpe_ratio, sortino_ratio, win_rate, profit_factor, config_info, output_dir):
    """
    ✅ v82: Aggiornata per includere nuove metriche
    """
    if trades_df is None or trades_df.empty:
        logging.warning(f"Nessun trade da salvare per {symbol} - {strategy_name} - Top {rank}")
        return

    q_buy_label = str(config_info.get('quantile_buy', 'N/A')).replace('.', 'p')
    n_features_label = config_info.get('features', 'N/A')
    csv_filename = f"{symbol}_{strategy_name}_top{rank}_q{q_buy_label}_feat{n_features_label}_oos.csv"
    csv_path = os.path.join(output_dir, csv_filename)

    try:
        with open(csv_path, 'w', encoding='utf-8') as f:
            f.write(f"# Summary Metrics\n")
            f.write(f"Total_Return_Pct,Max_Drawdown_Pct,Calmar_Ratio,Sharpe_Ratio,Sortino_Ratio,Win_Rate_Pct,Profit_Factor,Window_Size,Num_Features,Quantile_Buy,Quantile_Short\n")
            f.write(f"{total_return:.2f},{max_dd*100:.2f},{calmar_ratio:.2f},{sharpe_ratio:.2f},{sortino_ratio:.2f},{win_rate:.2f},{profit_factor:.2f},{config_info['window']},{config_info['features']},{config_info['quantile_buy']},{config_info['quantile_short']}\n")
            f.write(f"\n")
...
def main():
            ...
            for n_features in config["FEATURE_COUNTS_TO_TEST"]:
                ...
                parallel_results = Parallel(n_jobs=n_jobs_validation, backend='loky')(
                    delayed(run_backtest_for_symbol)(
                        symbol, df_features_for_run, df_targets, df_regimes, df_close, df_high, df_low,
                        n_features, window_size_val, q_buy, q_short,  # ✅ v82: Passa quantili
                ...
                try:
                    ...
                    logging.error(f"ERRORE salvataggio checkpoint di validazione: {e}")
    ...
    for symbol in config["TICKERS"]:
        ...
        for strategy_type, candidates in candidate_strategies[symbol].items():
            ...
            if strategy_type == 'trend_following':
                logging.info(f"[{symbol}] Salto la strategia 'Trend Following' come da v82.")
                continue
            ...
            for i, cand_config in enumerate(candidates):
                ...
                q_buy, q_short = cand_config['quantile_buy'], cand_config['quantile_short']
                ...
                # ✅ v82: Passa quantili come parametri
                oos_result = run_backtest_evaluation(
                    ...
                    quantile_buy=q_buy,  # ✅ v82
                    quantile_short=q_short  # ✅ v82
                )
        ...
        for strategy_type, results_list in all_oos_results[symbol].items():
            ...
            for rank, (cand_config, result) in enumerate(valid_results[:config['TOP_N_CANDIDATES_FOR_CSV']], start=1):
                save_trades_to_csv(
                    ...
                    sharpe_ratio=result.get('sharpe_ratio', 0),  # ✅ v82
                    sortino_ratio=result.get('sortino_ratio', 0),  # ✅ v82
                    win_rate=result.get('win_rate', 0),  # ✅ v82
                    profit_factor=result.get('profit_factor', 0),  # ✅ v82
    ...
    with open(final_report_path, 'w', encoding='utf-8') as f:
        all_bh_tickers = list(set(config["TICKERS"] + ['AAPL', 'GOOG', 'TSLA', '^IXIC', 'MRNA', 'LLY', 'ETH-USD', 'SOL-USD']))
        for symbol in all_bh_tickers:
            if symbol not in df_close.loc[oos_idx].columns:
                continue
            ...
            if symbol not in all_oos_results:
                f.write("Nessun risultato di strategia per questo asset (solo B&H).\n")
                continue
            ...
            for strategy_type, results_list in all_oos_results[symbol].items():
                ...
                sharpes = [res['sharpe_ratio'] for res in flat_results if res and res.get('sharpe_ratio') is not None]
                sortinos = [res['sortino_ratio'] for res in flat_results if res and res.get('sortino_ratio') is not None]
                win_rates = [res['win_rate'] for res in flat_results if res and res.get('win_rate') is not None]
                profit_factors = [res['profit_factor'] for res in flat_results if res and res.get('profit_factor') is not None]
                ...
                f.write(f"  - Performance dei candidati:\n")
                f.write(f"    * Calmar Ratio - Media: {np.mean(calmars):.2f}, Mediana: {np.median(calmars):.2f}, Min: {np.min(calmars):.2f}, Max: {np.max(calmars):.2f}\n")
                f.write(f"    * Ritorno % - Media: {np.mean(returns):.2f}%, Mediana: {np.median(returns):.2f}%\n")

                # ✅ v82: Nuove metriche nel report
                if sharpes:
                    f.write(f"    * Sharpe Ratio - Media: {np.mean(sharpes):.2f}, Mediana: {np.median(sharpes):.2f}\n")
                if sortinos:
                    f.write(f"    * Sortino Ratio - Media: {np.mean(sortinos):.2f}, Mediana: {np.median(sortinos):.2f}\n")
                if win_rates:
                    f.write(f"    * Win Rate % - Media: {np.mean(win_rates):.2f}%, Mediana: {np.median(win_rates):.2f}%\n")
                if profit_factors:
                    f.write(f"    * Profit Factor - Media: {np.mean(profit_factors):.2f}, Mediana: {np.median(profit_factors):.2f}\n")

    logging.info(f"Report finale OOS salvato in: {final_report_path}")
    logging.info(f"\n{'='*30} BACKTEST v82 COMPLETATO {'='*30}\n")
    logging.info(f"Tutti i risultati sono stati salvati in: {output_dir}")
